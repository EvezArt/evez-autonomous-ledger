...
def get_pulse():
    r = requests.get(f"https://api.github.com/repos/{OWNER}/{REPO}/contents/runtime/pulse.json", headers=GH_HEADERS)
...
def update_pulse_with_airtable_id(record_id):
    ...
    requests.put(f"https://api.github.com/repos/{OWNER}/{REPO}/contents/runtime/pulse.json",
        headers=GH_HEADERS, json={"message": f"🗂 airtable sync confirmed: {record_id}", "content": content, "sha": sha})
